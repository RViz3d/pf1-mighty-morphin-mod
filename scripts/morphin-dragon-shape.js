...
/**
 * Application for selecting a shape from the Dragon Shape spell to change into and then applying that shape to an actor
 */
export class MorphinDragonShape extends FormApplication {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            popOut: true,
            template: 'modules/pf1-mighty-morphin/templates/dragonShapeDialog.html',
            id: 'mighty-morphin-dragonShape',
            title: 'Mighty Morphin Dragon Shape',
            resizable: false,
            width: 550
        });
    }
    ...
    async getData() {
        ...
        // Set the default size to the largest available dragon
        ...
        // Create radio button data for dragon sizes, set the one for the default size as the default checked button
        ...
        // Get the dragon that will be the first shown in the form dropdown and build the preview of the changes the form will provide
    ...
    async applyChanges(event, chosenForm) {
        ...
        for (let i = 0; i < MorphinChanges.changes[chosenForm].attacks.length; i++) {
            ...
            if (!!attack.special) {
                for (let j = 0; j < attack.special.length; j++) {
                    ...
                    if (!MorphinDragonShape.allowedSpecials[this.level].includes(specialElement)) {
        ...
        for (let item of armorAndShields) {
            ...
            if (!!originalArmor) {
                ...
                // take off armor if it's not wild armor or this is not dragon shape from wild shape
    ...
    /**
     * Processes the changes from the selected dragon form into a readable preview display html. In the process it validates the changes based on the level of the spell
     * 
    ...
    async buildPreviewTemplate(chosenForm) {
        ...
        data.scoreChanges = '';
        this.changes = MorphinChanges.changes.dragonShape.dragon[this.chosenForm.size].changes;
        for (let i = 0; i < this.changes.length; i++) {
            const change = this.changes[i];
        ...
        for (let i = 0; i < attackList.length; i++) {
            ...
            if (!!element.special) { // process any specials associated with the attack
                for (let j = 0; j < element.special.length; j++) {
                    ...
                    if (MorphinDragonShape.allowedSpecials[this.level].includes(specialName)) {
        ...
        for (let i = 0; i < specialAttackList.length; i++) {
            ...
            for (let j = 0; j < (element.special?.length || 0); j++) {
                ...
                if (!MorphinDragonShape.allowedSpecials[this.level].includes(special)) valid = false;
            }
        ...
        for (let i = 0; i < this.senses.length; i++) {
            ...
            if (senseEnumValue >= MorphinChanges.SENSES.DARKVISION70.value && senseEnumValue <= MorphinChanges.SENSES.DARKVISION120.value) {
                if (this.level < 3) this.senses[i] = Math.min(senseEnumValue, MorphinChanges.SENSES.DARKVISION60.value);
            }
        ...
        for (let i = 0; i < this.special.length; i++) {
            ...
            if (!MorphinDragonShape.allowedSpecials[this.level].includes(specialName.split(' ')[0])) {
        ...
        <p><span class="previewLabel">Energy Resistances: </span><span id="di">' + data.eres + '</span></p>
